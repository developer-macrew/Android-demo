package com.example.dashboard.exception;

public @interface Constants {

    String USER = "user";
    String DATE_FORMAT="dd-MM-yyyy";
    String FORMAT_EEE_dd_MM="EEE'\n'dd/MM";
    String DATE_FORMAT_DD_MM="dd MMMM";
    String TIME_FORMAT="hh:mm:ss";
    String ENTER_EMAIL="Please enter registered E-mail/Username";
    String ENTER_VALID_EMAIL="Please enter valid E-mail";
    String ENTER_PASSWORD="Please enter the password";
    String AUTH="Bearer";
    String AUTH_TOKEN="Basic UzYxMDQ1MzA6OWpATSQ3RFNHSXc1XjRO";
    String UN_AUTH_USER="Unauthorised User";

    String  FORBIDDEN="Forbidden";
    String  INTERNAL_ERROR="Internal Server Error";
    String  BAD_REQUEST= "Bad Request";
    String  API_NOT_RESPONDING="Something Went Wrong API is not responding properly!";
    String  CANNOT_CONNECT_TO_SERVER="Sorry cannot connect to the server!";
    String  DEPRECATION="DEPRECATION";

    String  NO_INTERNET="No Internet Connection";
    String  HTTP_CODE="http_code";
    String  MESSAGE="message";

    // PREFERENCE=============================================================
    String IS_USER_LOGIN = "IS_USER_LOGIN";
    String USER_ID = "USER_ID";
    String FirstName = "FirstName";
    String LastName = "LastName";
    String Email = "Email";
    String UserName = "UserName";
    String DeviceToken = "DeviceToken";
    String Phone = "Phone";
    String RoleId = "RoleId";
    String Status = "Status";
    String DepartmentId = "DepartmentId";
    String Address = "Address";
    String City = "City";
    String State = "State";
    String Zip = "Zip";
    String Country = "Country";
    String AvatarDirectory = "AvatarDirectory";
    String AvatarFilename = "AvatarFilename";
    String PROFILE_DATA = "PROFILE_DATA";
    String PROFILE_SPINNER_DATA = "PROFILE_SPINNER_DATA";

    String BearerToken = "BearerToken";
    String ClockStatus = "ClockStatus";

    String PREFERENCE_NAME = "ShowTec";
}
